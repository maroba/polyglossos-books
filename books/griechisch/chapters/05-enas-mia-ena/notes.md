# Pipeline-Protokoll — Kapitel 5: enas-mia-ena

- Lauf: /new-chapter griechisch 5, gestartet 2026-06-07
- Curriculum: artikel-unbestimmt-nom-sg (ένας/μία/ένα) im Nominativ Singular
  (+ recycle artikel-genus-nom-sg, praesens-omega-sg)
- vocab_target 16, vocab_recycle_min 7; Themen Café/Getränke/Gegenstände/Bestellen
- cultural_topic: Bestellkultur — was man im Kafenío typischerweise trinkt
- can_do: unbestimmten Artikel nach Genus wählen; im Café eine Sache benennen und
  erbitten; nach dem Genus eines neuen Wortes fragen (Ένας, μία ή ένα;)

## Befunde

### Phase 1 (Schreiben — teacher, 2026-06-07)

- chapter.md (alle Sektions-Marker gesetzt: lernziele, dialog lang=el, vokabeln
  [leer], grammatik, uebungen [Platzhalter], landeskunde, loesungen [Platzhalter])
  und meta.yaml neu angelegt. Übungen + Lösungen bewusst leer für exercise-designer.

- **Grammatik-Aufbau** (Beispiele → Muster → Regel, wie Kap. 3/4):
  erst die drei Bestellungen aus dem Dialog (μία πορτοκαλάδα / ένα ποτήρι /
  ένας χυμός), dann die Parallel-Tabelle zu ο/η/το, dann die Genus-Wahl-Regel.
  Vier Café-Gegenstände als Übungsmaterial im Fließtext eingebaut (το τραπέζι,
  η καρέκλα, το φλιτζάνι, ένα πιάτο) — kein Dreier-Bullet-Block.

- **Kasus-Spezialfall behutsam gelöst** (zentrale Curriculum-Auflage): Eigener
  Abschnitt „Die alte Bestellung wiedergesehen: έναν καφέ" stellt ένας χυμός
  (Nom., NEU systematisiert) neben έναν καφέ (Bestell-Chunk aus Kap. 2, Akk.).
  Erklärt über „benennen" (Grundform ένας) vs. „bestellen/wollen" (έναν, nur
  beim männlichen Wort verändert; weiblich/sächlich bleiben gleich) — OHNE das
  Wort Akkusativ aktiv zu lehren oder ein Paradigma aufzubauen. Ausdrücklich:
  „nur wiedererkennen, nicht bilden; das System heißt Kasus und kommt später."
  Der Dialog hält das Bestellen über den bekannten Chunk (έναν καφέ;) und die
  neuen Nominativ-Benennungen (μία πορτοκαλάδα, ένα ποτήρι νερό, ένας χυμός εκεί;).

- **Genus-Frage als Can-do** ohne das Verb „sein": Da είμαι/είναι laut Curriculum
  erst Kap. 6 kommt, ist die Genus-Frage NICHT als „Τι είναι αυτό;" formuliert
  (das hätte είναι + Demonstrativ αυτό vorausgesetzt — beides unzulässig).
  Stattdessen über den Artikel selbst: „Ένας, μία ή ένα;" — passt sogar enger
  zum Kapitelfokus. Dafür ή (oder) neu eingeführt.

### Vokabel-Bilanz

- **NEU eingeführt (15; Richtwert 16, innerhalb ±20 %), je mit Genus:**
  - μία (art, f) — unbestimmter Artikel weiblich (Grammatik-Kern; ένας/ένα/έναν
    sind bereits Kap. 2 deklariert → recycelt, nicht neu).
  - Getränke: πορτοκαλάδα (f), λεμονάδα (f, rezeptiv/Landeskunde), χυμός (m),
    μπίρα (f), γάλα (n, rezeptiv/Landeskunde), τσίπουρο (n),
    ελληνικός (adj, rezeptiv/Landeskunde — „ελληνικός καφές", NICHT als
    flektierbares Adjektiv gelehrt; Adjektive erst Kap. 8).
  - Gegenstände: ποτήρι (n), πιάτο (n), τραπέζι (n), καρέκλα (f), φλιτζάνι (n).
  - Café/Funktion: μενού (n), ή (conj).
  - Genus-Kontraste deutsch≠griechisch bewusst gesetzt: η μπίρα (dt. das Bier),
    το τραπέζι (dt. der Tisch), η καρέκλα (dt. der Stuhl), το φλιτζάνι (dt. die
    Tasse) — knüpft an die Falle aus Kap. 4 an.
  - Geplant, aber NICHT aufgenommen: σερβιτόρος (kam im Text nicht vor; nicht
    deklariert, um keine Box-Vokabel ohne Textbeleg zu erzeugen).

- **RECYCELT (Vorgabe min. 7; deklariert 25):** ο/η/το, ένας/ένα/έναν+καφέ(ς),
  νερό, φραπές (alle Kap. 2); και, πολύ, ωραία, ορίστε, ναι, όχι, παρακαλώ,
  ευχαριστώ (Kap. 1/2); τι, κάνω (Τι κάνεις;), καλά, εδώ, λοιπόν, τώρα (Kap. 3);
  εκεί, κύριος (Kap. 3/4); καλησπέρα (Kap. 1). Grammatik-Recycling:
  artikel-genus-nom-sg (ο/η/το als Genus-Anker für die unbestimmte Wahl) und
  praesens-omega-sg (Τι κάνεις; im Dialog).

### Eiserne-Regel-Selbstprüfung

- **Nur Bekanntes + Neudeklariertes + Eigennamen.** Alle griechischen Tokens
  manuell gegen kumulatives Vokabular (Kap. 1–4) + die 15 Neudeklarationen
  abgeglichen. Restbefund: keiner. Eigennamen: Λένα, Νίκος/Νίκο, Κώστας (κύριος
  Κώστας) — erlaubt.
- **Beschränkung auf Nominativ Singular** eingehalten: kein Plural, kein
  aktives Akkusativ-/Genitiv-Paradigma. Der Bestell-Chunk έναν καφέ erscheint
  als bekannter Block (Kap.-2-Niveau), nicht als neue Morphologie; explizit als
  „nur wiedererkennen" markiert. καφέ ist deklarierte inflected_form von καφές.
- **Keine Transliteration.** Ursprüngliche Aussprache-Umschrift „mí-a" entfernt
  und verbal umformuliert. Griechisches Pluralwort „μεζέδες" aus der
  Landeskunde gestrichen (Plural erst Kap. 15; Umschrift unzulässig) → jetzt
  „kleine, geteilte Häppchen". Stehen geblieben: freddo (eingebürgerter
  Café-Markenbegriff, keine griechische Umschrift), Ouzo, Kafenío (buchweit so).
- **Endungs-Hervorhebung** nur am ganzen, deklarierten Wort: „ένα**ς** trägt ein
  zusätzliches Schluss-Sigma" — kein freistehendes Endungsfragment. Tokenizer
  entfernt ** vor dem Matchen → ένας als ganzes Token (deklariert); Lookbehind
  blockiert ohnehin Fragmente nach Bindestrich.
- **Genusregel ans Schriftbild geknüpft** („der bestimmte Artikel oder die
  geschriebene Endung verrät das Genus"), konsistent mit Kap. 4 („das Auge
  entscheidet"). ή (Akzent!) ausdrücklich vom Artikel η über den Akzent
  abgegrenzt.
- **Akzente** durchgängig Monotoniko geprüft. Metasprache Deutsch.
- Hinweis für die Prüfphase: continuity_check.py / validate_schema.py konnte ich
  mangels Ausführungsrecht nicht selbst laufen lassen; Abgleich manuell.

### Phase 2 (Übungen) — erledigt (exercise-designer, 2026-06-07)

- **7 Übungen, Progression rezeptiv → frei** (Vorbild Kap. 3/4):
  1. article-recognition — ένας/μία/ένα ankreuzen (rezeptiv, mit Beispiel-Item).
  2. genus-sorting — drei Spalten (ένας/μία/ένα) nach Genus befüllen.
  3. gap-fill — ένας/μία/ένα aktiv einsetzen (Kerntyp des Kapitels).
  4. contrast-multiple-choice — deutsche Genus-Falle (το τραπέζι, η καρέκλα,
     το φλιτζάνι; Item 4–6 als ehrliche „Treffer", damit man nicht aufs
     Widerspruchs-Muster rät).
  5. definite-indefinite-transform — ο/η/το → ένας/μία/ένα: Recycling von
     artikel-genus-nom-sg (Kap. 4) und Zusammenführung best./unbest. Artikel.
  6. communicative-cafe-ordering — Café-Bestellung; benennen (ένας/μία/ένα) vs.
     bestellen (έναν καφέ nur als Kap.-2-Chunk, NICHT aktiv flektiert).
  7. free-production — eigene Bestellung + Benennung, Musterlösung + Varianten.

- **Recycling-Auflagen abgedeckt:** Übung 5 ist reine Wiederholung älteren
  Materials (ο/η/το aus Kap. 4); Dialog-Figuren/Schauplatz weiterverwendet
  (Níkos, Kafenío, Tablett, Nebentisch mit τσίπουρο).

- **Eiserne-Regel-Selbstprüfung Übungen:**
  - Nur Vokabular Kap. 1–4 + Kap.-5-Neudeklarationen. Distraktoren ausschließlich
    bekannte Artikelformen (ένας/μία/ένα). Alle Tokens manuell abgeglichen.
  - Durchgehend Nominativ Singular. Kein Plural, kein aktives Akkusativ-Paradigma.
    έναν καφέ erscheint nur in Üb. 6 als bekannter Bestell-Block, ausdrücklich als
    „nur wiedererkennen, nicht bilden" gelöst (Lösung erklärt έναν vs. ένας).
  - Keine Transliteration; griechische Schrift, Monotoniko geprüft. ή (oder)
    kommt in den Übungen nicht vor; η als Artikel klar von ή getrennt.
  - Lückenstruktur eindeutig (Lehre aus Kap. 4): jede Lücke mit ________ markiert,
    Beispiel-Item bei jedem neuen Übungstyp; bei Üb. 6 explizit benennen vs.
    bestellen vorgegeben.
  - Endungs-Hervorhebung nur am ganzen, deklarierten Wort (ένα**ς** o. Ä. nicht
    isoliert in deutscher Prosa).
